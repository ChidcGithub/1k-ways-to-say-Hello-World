---
id: 0402
language: brainfuck
paradigm: esoteric
output: stdout
platform: cli
era: classic
tags: ["one-liner"]
notes: Single-line Brainfuck source with no explicit line breaks
author: ChidcGithub
date: 2026-07-20
---

# #0402 · Brainfuck oneliner

## Description

A Brainfuck Hello World entry: Single-line Brainfuck source with no explicit line breaks

## Source Code

hello.bf:

```brainfuck
++++++++++[>+++++++>++++++++++>+++>+>++++<<<<<-]>++.>+.+++++++..+++.>>>++++.<<++.<<+++++++++++++++.>.+++.------.--------.>+.>.
```

## Run

Brainfuck has no standard runtime. Use a Brainfuck interpreter (e.g. beef, bf).

```bash
cd hello-worlds/brainfuck/0402-oneliner
beef hello.bf
```

## Categorization

| Dimension | Value | Reason |
|-----------|-------|--------|
| language | brainfuck | Brainfuck, esoteric 8-instruction language (Urban Müller, 1993) |
| paradigm | esoteric | Brainfuck's instruction set is its own paradigm |
| output | stdout | Output via brainfuck . instruction |
| platform | cli | Runs in a cli context |
| era | classic | Brainfuck created in 1993 (classic era) |
| tags | ["one-liner"] | Single-line Brainfuck source with no explicit line breaks |

## Reference

- [Brainfuck — Wikipedia](https://en.wikipedia.org/wiki/Brainfuck)
